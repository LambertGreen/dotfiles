...
$stowCheck = & $bashPath -c "export PATH=/usr/bin:/mingw64/bin:\$PATH; which stow 2>/dev/null"
if ($stowCheck) {
    Write-Host "Stow is already installed in MSYS2" -ForegroundColor Yellow
    & $bashPath -c "export PATH=/usr/bin:/mingw64/bin:\$PATH; stow --version"
    exit 0
}
...
try {
    & $bashPath -c "export PATH=/usr/bin:/mingw64/bin:\$PATH; pacman -S --noconfirm stow"
} catch {
