...
*Update (6/13/2022)*:
I updated WSL on my home Windwos machine to use =Arch Linux= which uses =Pacman=, which has new enough Emacs version, however we still need the =Nix= option for the work =CentoOS 7= system.
...
*Update (6/13/2022)*: Actually it is prefefered to use =Pacman= packages for =Emacs=, so =Scoop= is again preferred over =Chocolatey=. Note: =Scoop= also supports installing applications system wide rather than per user which can provide a shorter path prefix for =Msys= installation.
#+begin_src ps
...
** Windows StartMenu
Copy the files from ~./windows_startmenu/programs~ to ~C:/Users/Lambert/AppData/Roaming/Microsoft/Windows/Start Menu/Programs~
...
*Update (3/31/22)*: there is a long standing issue with the Windows SSH. Recommendation is to install OpenSSH via =Scoop=/=Choco=.
** TODO Add a one time bat file for setting up =Windows= ~PATH~ variable
Maybe this needs to be a manual step?
Anyway we want to make sure that the ~SYSTEM~ ~PATH~ has often used programs such as =Git=, =Ssh=, etc. set.
