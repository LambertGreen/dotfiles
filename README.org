...
One each OS platform there are multiple options for package management. On Linux there is often a system package manager that comes with the specific Linux distribution, and on the other hand for macOS/Windows, there is no decent command-line based system package manager, but instead a GUI based app store. My preference is =Homebrew= for macOS/Linux and =Scoop= for Windows, because they both allow usage without the need for root/admin priviledges on the machine.
...
** TODO Add package manager export files for your machines [/]
   Let's add these as a quick backup of existing installed applications.
*** [ ] Add =Brewfile= for MacBook Air
*** [ ] Add =Brewfile= for MacBook Pro
*** [ ] Add =Scoop= export file for Windows Gaming rig
*** [ ] Add =Brewfile= for Ubuntu WSL machine
