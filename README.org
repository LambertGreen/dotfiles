...
*Note* I moved from using =Scoop= to =Chocolatey= to avoid =Scoop='s shims and because =Chocolatey= installs in ~C:/toos/msys2~ which is a preferred shallower directory structure.
#+begin_src sh
choco install msys2
#+end_src
